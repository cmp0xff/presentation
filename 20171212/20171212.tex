...
$\chi$ will be called the first, second Friedmann eqs.\ and the
...
\item For $p_\beta \neq 0$, taking the gauge $\overline{N} =
-6\mscrs\sqrt{\varkappa}\dot{\beta}/p_\beta$, the first Friedmann equation
can be integrated
\begin{align}
...
\end{cases}
\end{align}
The last solution is not real, whereas the first three cases are consistent
with the Klein--Gordon equation.

\item The trajectory equation for $\rbr{+,-}$ and $\rbr{-,+}$ contains two
and infinite distinct solutions, respectively.
...
or $\rbr{-,+}$ to guarantee $\overline{N} > 0$, which is automatically
consistent with the Klein--Gordon equation. Taking $\overline{N} =
\rbr{2\varkappa^2\vbr{V}}^{-1/2}$ yields
\begin{align}
\ee^{\mscrs_\chi g \chi} = \cbr{2\kappa/g\rbr{t-t_0}}^2.
\end{align}
...
\begin{align}
H^\text{p} &= \overline{N}H_\perp + P_{\overline{N}} V_{\overline{N}},
\\
H_\perp &= -\mscrs\frac{p_\beta^2}{12\varkappa^{1/2}}
+\mscrl\mscrs\frac{p_\chi^2}{2\varkappa^{3/2}}
+\varkappa^{3/2}V\ee^{\mscrs_\chi g \chi}.
\end{align}
\end{itemize}
