...
\end{align}
\item
\Cref{eq:WDW-10} is a Klein--Gordon-like equation, hyperbolic for
...
as $\alpha \to +\infty$
\item $\rbr{-,+}$: $\vbr{\rfun{\mathrm{Y}_{n}}{\sigma}} \to +\infty$
as $\alpha \to -\infty$, $\forall n \in \BbbZ$.
\begin{itemize}
\item
for $\nu \in \BbbR\backslash\BbbZ$, one can choose $\mathrm{J}_{-\nu}$ as
another base instead of $\mathrm{Y}_{+\nu}$, since $\mathrm{J}_{\pm\nu}$
...
\begin{frame}%
{Introducint norm for the wave functions}%
{233}
\begin{itemize}
\item To make sense of and construct a wave packet, defining a norm is
necessary
\item A common starting point of most approaches is the 
\alert{Schrödinger norm}
\begin{align}
\rbr{\Psi_1,\Psi_2}_\text{S} \coloneqq
\int \dd \chi\,\rfun{\rho_\text{S}}{\beta,\chi},\qquad
\rho_\text{S} \coloneqq \Psi_1^* \Psi_2.
\end{align}
\item Manifestly $\rho_\text{S} \ge 0$. However, there is no continuity
equation $\dot{\rho}_\text{S} + \nabla\cdot\vec{j}_\text{S} = 0$ for
$\rho_\text{S}$, because \cref{eq:WDW-10} is KG-like and not heat-like.

\end{itemize}
\end{frame}

\begin{frame}%
{Self-adjointness of operators for the phantom model}%
{$\rbr{+,-}$}
\begin{itemize}
