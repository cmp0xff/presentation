...
\end{align}
where $\varkappa = 8\pp\nG$, $\dd\Omega_3^2$ is the dimensionless spacial 
metric, and the scale factor is $a = \varkappa^{-1/2}\ee^\alpha$.
\item Homogeneous Klein--Gordon field with an exponential potential 
$V\ee^{\lambda\phi}$ (dubbed Liouville), $\lambda, V\in \BbbR$.

\item Total action
\begin{align}
\mscrS &= S_{\text{Einstein--Hilbert}} + S_\text{Gibbons--Hawking--York} + 
S_\text{Liouville} = \Omega\int\dd t\,L \nonumber \\
&\coloneqq \Omega\int\dd t\, \varkappa^{3/2}N\ee^{3\alpha}
...
\end{align}
the effective Lagrangian transforms to
\begin{align}
...
\item
\begin{align}
\Delta \coloneqq \lambda^2 - 6\mscrl\varkappa,\quad
s \coloneqq \sgn \Delta,\quad
g \coloneqq \sqrt{\vbr{\Delta}} \equiv \sqrt{\mscrs\Delta},
\end{align}
